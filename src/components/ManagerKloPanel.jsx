...
// Manager-only chat surface. Lives inside the Team page and reuses the same
// Klo voice the seller hears, but with pipeline-level context.
export default function ManagerKloPanel({ team, pipeline }) {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    let mounted = true
    async function init() {
      const t = await ensureManagerThread({ teamId: team.id, managerId: user.id })
      if (!mounted || !t) return
      setThread(t)
  ...
  // it lands, just like per-deal chat.
  useEffect(() => {
    if (!thread?.id) return
    const channel = supabase
      .channel(`mgr-thread-${thread.id}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
