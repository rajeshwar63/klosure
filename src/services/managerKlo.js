...
// whole pipeline (every active deal + commitments + recent activity) and
// hands it to Claude Sonnet 4.6 with a manager-flavoured persona.
//
...
function stubReply({ question, pipeline }) {
  ...
  const overdue = active.filter((d) => d.overdueCount > 0)
  ...
  if (q.includes('risk') || q.includes('red') || q.includes('losing')) {
    ...
    const top = red[0]
    return `${red.length} deal${red.length === 1 ? '' : 's'} red — ${formatCurrency(valueAtRisk)} at risk. Top: ${top.title} (${top.seller_name}, ${top.overdueCount} overdue). Push there today.`
  }
  ...
  if (q.includes('overdue') || q.includes('miss')) {
    if (overdue.length === 0) return `No overdue commitments across the team. Stay on it.`
    return `${overdue.length} deal${overdue.length === 1 ? '' : 's'} with overdue commitments. ${overdue[0].seller_name}'s ${overdue[0].title} has ${overdue[0].overdueCount}.`
  }
  ...
  return `${active.length} active · ${red.length} red · ${amber.length} amber. Tell me what to dig into — risk, overdue, a specific rep — and I'll point you at the move.`
}
