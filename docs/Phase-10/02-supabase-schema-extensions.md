# Sprint 02 — Supabase schema extensions

**Sprint:** 2 of 11
**Estimated:** 1 day
**Goal:** Add the database tables Phase A needs: `nylas_grants`, `email_events`, `meeting_events`, `meeting_usage`, `team_pool`. Plus indexes, RLS, and helpers. No code outside SQL.

## Why this matters

These tables are the durable memory for everything Nylas tells us. The webhook handler (sprint 04) writes to them; extraction pipelines (5, 6) read from them; metering (7) and dashboards (10) aggregate from them. Get the shape right once and the rest of the spec is straightforward queries. Get it wrong and we're doing painful migrations during week 3.

The roadmap's What-Ships list mentions only `nylas_grants`, `meeting_usage`, `team_pool`. We add `email_events` and `meeting_events` because without them we have no audit trail of what Nylas told us — which we need for debugging, replay, and the manager's "what changed today" view in Phase B.

## Deliverable

A single SQL file: `supabase/phase_a.sql`. Idempotent (uses `if not exists` everywhere). Apply with:
...
create table if not exists public.meeting_events (
  ...
  meeting_provider text
    check (meeting_provider in ('zoom', 'meet', 'teams', 'other', null)),
  -- Routing:
  deal_id uuid references public.deals(id) on delete set null,
...
-- team_pool: team manager (owner) and all team members can read; service role writes.
...
# Sanity-check the new tables exist:
supabase db remote-commit --dry-run    # if you use the migration workflow
# OR connect via psql and:
\d nylas_grants
\d email_events
\d meeting_events
\d meeting_usage
...
# Verify the helpers compile:
select * from public.get_team_pool('00000000-0000-0000-0000-000000000000');
-- Should return zero rows (no such team), no error.
```

## Acceptance

- [ ] `supabase/phase_a.sql` exists and applies cleanly via `supabase db push`
- [ ] All five tables visible in Supabase dashboard
- [ ] All RLS policies show "Enabled" in the dashboard
- [ ] `team_pool` rows exist for every team (run `select count(*) from team_pool` and compare to `select count(*) from teams`)
- [ ] `get_team_pool(<your team uuid>)` returns a row with `seat_count` matching your `team_members` count
- [ ] `increment_meeting_usage(...)` works end-to-end with a test invocation:
  ```sql
  -- Replace with real UUIDs from your dev environment:
  select * from increment_meeting_usage(
    '<team-uuid>',
    '<user-uuid>',
    null,
    30
  );
  -- Should return 1 row with new_used=30 and reasonable totals.
  ```
- [ ] `select * from team_pool where current_meeting_minutes > 0` shows the test increment
- [ ] Manually delete the test usage row before moving on:
  ```sql
  delete from meeting_usage where meeting_event_id is null;
  update team_pool set current_meeting_minutes = 0 where current_meeting_minutes > 0;
  ```

## Pitfalls

- **`generated always as` on `consumed_year_month`** is Postgres 12+. Supabase is on 15+. Fine.
- **The `for update` lock** in `increment_meeting_usage` is intentional. Without it, two simultaneous webhooks for two ending meetings could both read the same `current_meeting_minutes` and one increment is lost.
- **RLS policies use `manages_seller(uuid)`** — that helper is from `phase4.sql`. If for some reason it's not defined, this migration fails. Check `\df manages_seller` first.
- **Don't add an `updated_at` trigger to `meeting_events`** — webhooks are the only writer, and they set `updated_at` explicitly. A trigger would just add cycles.

→ Next: `03-oauth-flow-frontend.md`
