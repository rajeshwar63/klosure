# Step 04 — Bootstrap prompt

**Goal:** Write the one-time prompt that creates `klo_state` from a deal's full chat history. Used when a deal has `klo_state IS NULL` (legacy deals on first chat turn after migration).

## Deliverable

A new file: `supabase/functions/_shared/prompts/bootstrap-prompt.ts`
...
- The `extractionRulesText` import comes from a small file (next paragraph) that just exports the markdown content from `EXTRACTION_RULES.md` as a string. This keeps the rules in one place but usable in multiple prompts.

## Also create this small helper
...
[paste the full markdown content of supabase/functions/_shared/EXTRACTION_RULES.md here as a template literal]
`;
```

(Yes, this is a deliberate copy. We're prioritizing simple deploys over file-system reads at runtime. When you update the rules, update both files in the same commit.)

## Acceptance

- Both files created
- `bootstrap-prompt.ts` compiles with no errors
- `extraction-rules-text.ts` contains the rules from step 03
- Committed and pushed

→ Next: `05-extraction-prompt.md`
